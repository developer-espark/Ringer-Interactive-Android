...
public class CallHelper {
    ...
    private static final String ID_PREFIX = CallHelper.class.getSimpleName() + "_";
...
//                    call.unregisterCallback(mTelecomCallCallback);
     ...
     */
    CallHelper(int state) {
        mTelecommCall = null;
        mId = ID_PREFIX + Integer.toString(sIdCounter++);
        setState(state);
    }

    public CallHelper(android.telecom.Call telecommCall) {
...
//            Log.e(this, "CallExtras is corrupted, ignoring exception", e);
    ...
    protected void updateFromCallExtras(Bundle callExtras) {
        ...
        // CallHelper subject is present in the extras at the start of call, so we do not need to
...
//                + mSessionModificationState);
    ...
    public static boolean areSame(CallHelper call1, CallHelper call2) {
    ...
    public static boolean areSameNumber(CallHelper call1, CallHelper call2) {
        if (call1 == null && call2 == null) {
            return true;
        } else if (call1 == null || call2 == null) {
            return false;
